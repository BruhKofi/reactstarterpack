...
Now you can rename `src/Layout.css` to `src/App.scss` and run `npm run watch-css`. The watcher will find every Sass file in `src` subdirectories, and create a corresponding CSS file next to it, in our case overwriting `src/Layout.css`. Since `src/App.js` still imports `src/Layout.css`, the styles become a part of your application. You can now edit `src/App.scss`, and `src/Layout.css` will be regenerated.
